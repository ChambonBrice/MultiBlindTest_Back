from MultiBlindTest_Back.Library.bdd_client import execute_sql


class Campagne:
    @staticmethod
    def get_levels(user_id, campagne_id=1):
        payload = execute_sql(
            """
            SELECT
                l.ID AS ID,
                l.LevelName AS LevelName,
                l.Difficulty AS Difficulty,
                COALESCE(COUNT(m.ID), 0) AS nb_music,
                le.etat AS etat,
                cl.Theme AS theme,
                cl.Score AS score
            FROM CampaignLevels cl
            JOIN Levels l ON l.ID = cl.LevelID
            JOIN Levels_Etat le ON l.ID = le.level_id
            LEFT JOIN Music m ON m.LevelsID = l.ID
            WHERE cl.CampaignID = ? AND le.user_id = ?
            GROUP BY l.ID, l.LevelName, l.Difficulty, le.etat, cl.Theme, cl.Score, cl.ID
            ORDER BY cl.ID ASC
            """,
            (campagne_id, user_id),
        )
        return [{
            'id': row.get('ID'),
            'nom': row.get('LevelName'),
            'name': row.get('LevelName'),
            'difficulty': row.get('Difficulty'),
            'nb_music': row.get('nb_music') or 0,
            'etat': row.get('etat'),
            'theme': row.get('theme'),
            'campaign_score': row.get('score') or 0,
        } for row in payload.get('rows', [])]

    @staticmethod
    def complete_level(user_id, level_id):
        execute_sql(
            "UPDATE Levels_Etat SET etat = 'completed' WHERE user_id = ? AND level_id = ?",
            (user_id, level_id),
        )
        current_payload = execute_sql('SELECT ID, CampaignID FROM CampaignLevels WHERE LevelID = ?', (level_id,))
        current_rows = current_payload.get('rows', [])
        if not current_rows:
            return
        current = current_rows[0]
        next_payload = execute_sql(
            'SELECT LevelID FROM CampaignLevels WHERE CampaignID = ? AND ID > ? ORDER BY ID ASC LIMIT 1',
            (current.get('CampaignID'), current.get('ID')),
        )
        next_rows = next_payload.get('rows', [])
        if next_rows:
            execute_sql(
                "UPDATE Levels_Etat SET etat = 'unlocked' WHERE user_id = ? AND level_id = ?",
                (user_id, next_rows[0].get('LevelID')),
            )

    @staticmethod
    def init_user_levels(user_id, campagne_id=1):
        payload = execute_sql(
            """
            SELECT l.ID
            FROM CampaignLevels cl
            JOIN Levels l ON l.ID = cl.LevelID
            WHERE cl.CampaignID = ?
            ORDER BY cl.ID ASC
            """,
            (campagne_id,),
        )
        for i, level in enumerate(payload.get('rows', [])):
            etat = 'unlocked' if i == 0 else 'locked'
            execute_sql(
                'INSERT OR IGNORE INTO Levels_Etat (user_id, level_id, etat) VALUES (?, ?, ?)',
                (user_id, level.get('ID'), etat),
            )

    @staticmethod
    def get_level_detail(user_id, level_id):
        payload = execute_sql(
            """
            SELECT
                l.ID AS ID,
                l.LevelName AS LevelName,
                l.Difficulty AS Difficulty,
                l.hint AS hint,
                l.stats AS stats,
                le.etat AS etat,
                COALESCE(COUNT(m.ID), 0) AS nb_music
            FROM Levels l
            JOIN Levels_Etat le ON l.ID = le.level_id
            LEFT JOIN Music m ON m.LevelsID = l.ID
            WHERE l.ID = ? AND le.user_id = ?
            GROUP BY l.ID, l.LevelName, l.Difficulty, l.hint, l.stats, le.etat
            """,
            (level_id, user_id),
        )
        rows = payload.get('rows', [])
        if not rows:
            return None
        row = rows[0]
        tracks_payload = execute_sql('SELECT ID, Name, PATH FROM Music WHERE LevelsID = ? ORDER BY ID ASC', (level_id,))
        return {
            'id': row.get('ID'),
            'nom': row.get('LevelName'),
            'name': row.get('LevelName'),
            'difficulty': row.get('Difficulty'),
            'etat': row.get('etat'),
            'timer': 60,
            'lives': 3,
            'hint': row.get('hint'),
            'stats': row.get('stats') or 0,
            'nb_music': row.get('nb_music') or 0,
            'tracks': [
                {'id': t.get('ID'), 'name': t.get('Name'), 'path': t.get('PATH')}
                for t in tracks_payload.get('rows', [])
            ],
        }

    @staticmethod
    def get_music(level_id):
        payload = execute_sql('SELECT ID FROM Music WHERE LevelsID = ?', (level_id,))
        return [row.get('ID') for row in payload.get('rows', [])]
